...
#ifdef _MSC_VER
static const MOVParseTableEntry mov_default_parse_table[53];
#else
...
static int mov_read_chpl(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        str[str_len] = 0;
#ifdef _MSC_VER
        ff_new_chapter(c->fc, i, rational, start, AV_NOPTS_VALUE, str);
#else
...
#ifdef _MSC_VER
    st->codec->channels = temp[acmod] + lfeon;
#else
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    int ret;
#ifdef _MSC_VER
	MOVAtom movAtom = { AV_RL32("root"), INT64_MAX };
#endif
...
#ifdef _MSC_VER
            mov_read_default(mov, s->pb, movAtom) < 0 ||
#else
