...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
        ...
        height = is->height;
#ifdef _MSC_VER
        width = ((int)(height * aspect_ratio)) & ~1;
        if (width > is->width) {
            width = is->width;
...
#ifdef _MSC_VER
        Sleep(is->audio_st && is->show_audio ? rdftspeed : 5); //FIXME ideally we should wait the correct time but SDLs event passing is so slow it would be silly
#else
...
# ifdef _MSC_VER
static const AVFilterPad inputs[] = {{ NULL }};
...
#ifdef _MSC_VER
    FFSinkContext ffsink_ctx = { PIX_FMT_YUV420P };
#else
