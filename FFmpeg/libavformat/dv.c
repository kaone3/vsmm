...
#ifdef _MSC_VER
        avctx->bit_rate = av_rescale_q(c->sys->frame_size, rational,
                                       c->sys->time_base);
#else
...
void dv_offset_reset(DVDemuxContext *c, int64_t frame_offset)
{
	...
	{
		rational.num = 8;
		rational.den = c->ast[0]->codec->bit_rate;
        c->abytes= av_rescale_q(c->frames, c->sys->time_base,rational);
	}
#else
...
#ifdef _MSC_VER
    s->bit_rate = av_rescale_q(c->dv_demux->sys->frame_size, rational,
